        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-3">
              ...
              <input
                {...register("title")}
                className="w-full bg-[#121826] border border-slate-800 rounded-xl px-4 py-3 text-sm text-white outline-none"
              />
            </div>
            ...
            <div className="col-span-2">
              ...
              <textarea
                ...
                className="w-full bg-[#121826] border border-slate-800 rounded-xl px-4 py-3 text-sm text-white outline-none h-32"
              />
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-3 bg-[#121826] p-4 rounded-xl border border-slate-800 h-48 overflow-y-auto">
                {leetcodeTags.map((tag) => (
                  <label
                    key={tag}
                    className="flex items-center gap-2 text-sm text-slate-300 cursor-pointer hover:text-white whitespace-nowrap"
                  >
                    <input
                      type="checkbox"
                      value={tag}
            ...
            <div>
              ...
              <input
                type="number"
                {...register("releaseDay", { valueAsNumber: true })}
                className="w-full bg-[#121826] border border-slate-800 rounded-xl px-4 py-3 text-sm text-white outline-none"
              />
            </div>
          </div>
          ...
          <div className="space-y-10">
              ...
              {vtFields.map((field, index) => (
                ...
                >
                  ...
                  <input
                    ...
                    className="bg-[#121826] border border-slate-800 p-2 rounded text-white text-xs"
                  />
                  <button
                    type="button"
            ...
            <section>
              ...
              {htFields.map((field, index) => (
                <div
                  key={field.id}
                  className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 border border-slate-800 rounded-xl bg-[#0C1220]"
                >
                  <div className="flex flex-col">
                    <label className="text-[10px] text-slate-500 uppercase mb-1">
                      Input (JSON)
                    </label>
                    <textarea
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <section>
                ...
                {scFields.map((field, index) => (
                  <div
                    key={field.id}
                    className="mb-4 p-4 border border-slate-800 rounded-xl bg-[#121826]"
                  >
                    <input
                      {...register(`startcode.${index}.language`)}
                      placeholder="Language (e.g. javascript)"
                      className="w-full bg-[#0C1220] border border-slate-800 p-2 rounded text-white mb-2"
                    />
                    <textarea
                      {...register(`startcode.${index}.initialcode`)}
                      placeholder="Initial Code"
                      className="w-full bg-[#0C1220] border border-slate-800 p-2 rounded text-white h-20"
                    />
                    <button
                      type="button"
                      onClick={() => removeSC(index)}
                ...
                <button
                  type="button"
                  onClick={() => appendSC({ language: "", initialcode: "" })}
                  className="text-indigo-400 text-sm"
                >
                  + Add Language
                </button>
              ...
              <section>
                ...
                <button
                  type="button"
                  onClick={() => appendRS({ language: "", code: "" })}
                  className="text-indigo-400 text-sm"
                >
                  + Add Solution
                </button>
              ...
              <section className="mt-8">
                ...
                <p className="text-slate-500 text-xs mt-2 italic">
                  This code will be injected into the main() function to parse
                  input and call the user's solution class.
                </p>
              </section>
                ...
                <div className="bg-[#121826] p-6 rounded-xl border border-slate-800 space-y-6">
                  ...
                  <div>
                    ...
                    {errors.constraints?.inputConstraints && (
                      <p className="text-rose-400 text-xs mt-1">
                        {errors.constraints.inputConstraints.message}
                      </p>
                    )}
                  </div>
                </div>
