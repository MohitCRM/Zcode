...
export default function Problems() {
  const [data, setdata] = useState({ problems: [], seasonId: '', activePhase: '' });
  ...
  return (
    <div className="bg-[#090D16] min-h-screen text-slate-200 antialiased relative overflow-x-hidden">
      ...
      <main className="mx-auto max-w-7xl px-6 py-12 relative z-10">
        ...
        {/* Table Header - Updated grid-cols to [70px_1fr_200px] */}
        <div className="bg-[#121826]/40 border border-slate-800/40 rounded-xl px-6 py-3.5 grid grid-cols-[70px_1fr_200px] gap-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
        ...
        <div className="flex flex-col gap-y-2 mt-2.5">
          {data.problems?.map((prob, index) => (
            <div 
              ...
              className="group bg-[#121826]/70 backdrop-blur-sm rounded-xl border border-slate-800/60 p-5 grid grid-cols-[70px_1fr_200px] gap-4 items-center shadow-lg hover:border-indigo-500/40 hover:bg-[#161F30] transition-all cursor-pointer"
            >
              {/* Status Indicator */}
