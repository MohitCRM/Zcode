...
const guestfetchallproblmes = async (req, res) => {
  try {
    ...
    const solvedIds =
      userSeasonalStats && userSeasonalStats.problemsSolved
...
const createproblem = async (req, res) => {
  try {
    ...
    if (!drivercode) throw new Error("Driver code is required");
    if (!round || !releaseDay) {
    ...
    for (const ele of referencesolution) {
      ...
      const jsonLib = fs.readFileSync(
        path.join(__dirname, "../../libs/json.hpp"),
        "utf-8",
      );
      ...
      try {
        ...
        const compile = await sandbox.commands.run(
          "cd /home/user && g++ -O2 -std=c++17 main.cpp -o main 2>&1",
          {
            timeout: 30,
          },
        );
        ...
        if (compile.exitCode !== 0) {
          ...
          console.error("STDOUT/STDERR:", compile.stdout);
          throw new Error(
            "Compilation failed. Check the server terminal for details.",
          );
        }
        ...
        for (const testCase of totalTestCases) {
          ...
          const run = await sandbox.commands.run(
            `cd /home/user && timeout -k 0.5s ${timeLimit}s ./main < input.txt`,
          );
          ...
          if (JSON.stringify(expected) !== JSON.stringify(actual)) {
            throw new Error(
              `Wrong Answer: Expected ${JSON.stringify(expected)}, Got ${JSON.stringify(actual)}`,
            );
          }
        }
      } finally {
    ...
    if (err.message && err.message.toLowerCase().includes("limit")) {
      return res
        ...
        .json({
          error:
            "Servers are currently at maximum capacity. Please wait a few seconds and try creating again!",
        });
    }
  ...
  try {
    ...
    if (err.message && err.message.toLowerCase().includes("limit")) {
      return res
        ...
        .json({
          error:
            "Servers are currently at maximum capacity. Please wait a few seconds and try updating again!",
        });
    }
    return res
      .status(400)
      .json({ error: "Error Occurred during updates: " + err.message });
  }
};
...
const problemfetch = async (req, res) => {
  ...
  try {
    ...
    if (!currentSeason) {
      return res
        .status(500)
        .json({ error: "Configuration error: Season context missing." });
    }
    ...
    const problem = await Problem.findOne({
      ...
      .select(
        "-hiddenTestCases -__v -createdAt -updatedAt -problemcreator -drivercode ",
      )
      .lean();
    ...
    if (!problem) {
      return res
        ...
        .json({
          error:
            "Problem not found, locked behind a future round, or unreleased!",
        });
    }

    // Fetch video solution
    const videoSolution = await SolutionVideo.findOne({
      problemId: pid,
    }).lean();
    if (videoSolution) {
...
const allproblemfetch = async (req, res) => {
  try {
    ...
    const problems = await Problem.find({
      ...
      .select(
        "_id title difficulty tags baseEloReward releaseDay penaltyWrongAnswer",
      )
      .sort({ releaseDay: 1 });
    ...
    return res
      .status(500)
      .json({
        error: "Failed to retrieve active problem set",
        details: err.message,
      });
  }
};
